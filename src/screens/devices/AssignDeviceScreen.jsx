...
export default function AssignDeviceScreen({navigation,route}) {
  ...
  useEffect(()=>{getShipments().then(r=>setList((r||[]).map(normalizeShipment).filter(s=>s.category!=="completed"))).catch(e=>Alert.alert("Unable to load shipments",e.message)).finally(()=>setLoading(false))},[]);
  async function submit(){try{setSaving(true);await assignDevice(deviceId,selected);Alert.alert("Assigned",`${deviceId} has been assigned to ${selected}.`,[{text:"OK",onPress:()=>navigation.goBack()}])}catch(e){Alert.alert("Assignment failed",e.message)}finally{setSaving(false)}}
  if(loading)return <Loader text="Loading shipments..."/>;
  return <View style={styles.screen}><TopBar title="Assign Device" navigation={navigation}/><ScrollView contentContainerStyle={styles.content}><Text style={styles.lead}>Assign <Text style={styles.bold}>{deviceId}</Text> to a shipment.</Text>{list.map(s=><TouchableOpacity key={s.id} onPress={()=>setSelected(s.id)} style={[styles.row,selected===s.id&&styles.selected]}><View style={{flex:1}}><Text style={styles.id}>{s.id}</Text><Text style={styles.sub}>{s.product} · {s.source} → {s.destination}</Text></View><View style={[styles.radio,selected===s.id&&styles.radioOn]}/></TouchableOpacity>)}<TouchableOpacity disabled={!selected||saving} style={[styles.button,(!selected||saving)&&{opacity:.45}]} onPress={submit}><Text style={styles.buttonText}>{saving?"Assigning...":"Confirm Assignment"}</Text></TouchableOpacity></ScrollView></View>
}
