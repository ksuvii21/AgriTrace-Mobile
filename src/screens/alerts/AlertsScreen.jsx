...
export default function AlertsScreen({navigation}) {
  const {role}=useAuth();const {t}=useLanguage();const [filter,setFilter]=useState("all"),[alerts,setAlerts]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState(null);
  ...
  const ack=async a=>{if(role!=="ADMIN"){Alert.alert(t("alerts_admin_action_title"),t("alerts_admin_action_body"));return}try{await acknowledgeAlert(a.id);await load()}catch(e){Alert.alert(t("alerts_ack_failed"),e.message)}};
