...
export default function ShipmentDetailsScreen({navigation,route}) {
  ...
  if(loading)return <Loader text="Loading shipment..."/>;
  if(error||!s)return <View style={styles.center}><Text style={styles.err}>{error?.message||"Shipment not found"}</Text><TouchableOpacity onPress={()=>navigation.goBack()}><Text style={styles.link}>Go back</Text></TouchableOpacity></View>;
  ...
  const stepNames=["Created","Device","Ready","Transit","Warehouse","Delivered"];
  ...
  return <View style={styles.screen}><TopBar title={s.id} navigation={navigation} rightIcon="ellipsis-horizontal"/><ScrollView contentContainerStyle={styles.content}>
    <LinearGradient colors={[COLORS.green,COLORS.blue]} style={styles.hero}><Text style={styles.heroId}>{s.id}</Text><Text style={styles.heroTitle}>{s.product}</Text><Text style={styles.heroBadge}>{statusLabel(s.status)}</Text><Text style={styles.qty}>Quantity: {s.quantity}</Text></LinearGradient>
    ...
    <View style={styles.sensorGrid}><SensorCard icon="thermometer" value={s.temperature==null?"—":`${s.temperature}°C`} label="Temperature" status={tempWarn?"WARNING":"SAFE"} warning={tempWarn}/><SensorCard icon="water-percent" value={s.humidity==null?"—":`${s.humidity}%`} label="Humidity" status={humWarn?"WARNING":"SAFE"} warning={humWarn}/><SensorCard icon="weather-windy" value={s.gasStatus} label="Gas" status={gasWarn?"WARNING":"SAFE"} warning={gasWarn}/><SensorCard icon="battery-high" value={s.battery==null?"—":`${s.battery}%`} label="Battery" status="GOOD"/></View>
    <View style={styles.sync}><View><Text style={styles.small}>Device</Text><Text style={styles.strong}>{s.device||"Not assigned"}</Text></View><View style={{alignItems:"flex-end"}}><Text style={styles.small}>Tracking ID</Text><Text style={styles.strong}>{s.trackingId||"—"}</Text></View></View>
    <View style={styles.actions}><Action icon="pulse-outline" text="Live Monitoring" onPress={()=>navigation.navigate("LiveMonitoring",{shipmentId:s.id})}/><Action icon="git-branch-outline" text="View Traceability" onPress={()=>navigation.navigate("Traceability",{shipmentId:s.id})}/><Action icon="qr-code-outline" text="Shipment QR" onPress={()=>navigation.navigate("ShipmentQr",{shipmentId:s.id})}/><Action icon="flag-outline" text="Report Problem" onPress={()=>Alert.alert("Report Problem","Issue reporting endpoint is not present in the uploaded backend yet.")}/></View>
  </ScrollView></View>
}
