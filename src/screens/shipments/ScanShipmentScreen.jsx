...
export default function ScanShipmentScreen({ navigation }) {
  ...
  const handleCode = async (raw) => {
    ...
    try {
      // DEVICE QR — e.g. AGR-NODE-001 or DEV-...
      if (mode === "device" || code.startsWith("AGR-NODE") || code.startsWith("DEV-")) {
        const device = await getDevice(code);
        setResult({ type: "device", data: device });
        return;
      }

      // SHIPMENT QR — either "AGT-2026-00001" or a full trace URL
      const trackingId = extractTrackingId(code);
      if (!trackingId) {
        throw new Error("Invalid shipment QR.");
      }

      const trace = await getPublicTrace(trackingId);
      setResult({ type: "shipment", data: trace });
    } catch (error) {
      console.log("QR ERROR:", error);
      Alert.alert(t("scan_not_recognized_title"), error?.message || t("scan_not_recognized_default"));
      setScanned(false);
    } finally {
      setLoading(false);
    }
  ...
  if (!permission.granted) {
    return (
      <View style={styles.permission}>
        <Ionicons name="camera-outline" size={50} color={COLORS.green} />
        <Text style={styles.permissionTitle}>{t("scan_permission_title")}</Text>
        <TouchableOpacity style={styles.permissionBtn} onPress={requestPermission}>
          <Text style={styles.permissionBtnText}>{t("scan_allow_camera")}</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={styles.dark}>
      ...
      <View style={styles.top}>
        ...
        <View style={styles.toggle}>
          {["shipment", "device"].map((x) => (
            <TouchableOpacity
              key={x}
              onPress={() => { setMode(x); setScanned(false); setResult(null); }}
              style={[styles.toggleBtn, mode === x && styles.toggleActive]}
            >
              <Text style={[styles.toggleText, mode === x && styles.toggleTextActive]}>
                {x === "shipment" ? t("scan_mode_shipment") : t("scan_mode_device")}
              </Text>
            </TouchableOpacity>
          ))}
        </View>

        <TouchableOpacity style={styles.circle} onPress={() => setTorch((previous) => !previous)}>
      ...
      <View style={styles.center}>
        ...
        <Text style={styles.hint}>
          {loading ? t("scan_checking_backend") : t("scan_hint")}
        </Text>
      </View>
      ...
      <View style={styles.bottom}>
        <TouchableOpacity
          style={styles.quick}
          onPress={() => Alert.alert(t("scan_gallery_title"), t("scan_gallery_body"))}
        >
          <Ionicons name="image-outline" size={21} color={COLORS.white} />
          <Text style={styles.quickText}>{t("scan_gallery_button")}</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={styles.simulate}
          disabled={loading}
          onPress={() => handleCode(mode === "device" ? "AGR-NODE-001" : "AGT-2026-00001")}
        >
          <Ionicons name="qr-code" size={24} color={COLORS.white} />
          <Text style={styles.simText}>{loading ? t("scan_checking") : t("scan_simulate")}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.quick} onPress={() => setManualOpen(true)}>
          <Ionicons name="keypad-outline" size={21} color={COLORS.white} />
          <Text style={styles.quickText}>{t("scan_enter_code")}</Text>
        </TouchableOpacity>
      </View>
      ...
      <Modal
        ...
        visible={!!result}
        animationType="slide"
        onRequestClose={() => { setResult(null); setScanned(false); }}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.sheet}>
            ...
            <View style={styles.resultIcon}>
              <Ionicons
                name={result?.type === "device" ? "hardware-chip-outline" : "qr-code-outline"}
                size={27}
                color={COLORS.forest}
              />
            </View>
            ...
            <Text style={styles.sheetTitle}>
              {result?.type === "device" ? t("scan_device_detected") : t("scan_trace_verified")}
            </Text>

            <Text style={styles.sheetSub}>
              {result?.type === "device" ? result?.data?.deviceId : result?.data?.trackingId}
            </Text>
            ...
            {result?.type === "device" ? (
              <Info
                rows={[
                  [t("scan_info_device"), result?.data?.deviceId],
                  [t("scan_info_status"), result?.data?.status],
                  [t("scan_info_battery"), result?.data?.battery != null ? `${result.data.battery}%` : "—"],
                  [t("scan_info_shipment"), result?.data?.currentShipmentId || t("scan_info_unassigned")],
                ]}
              />
            ) : (
              // SHIPMENT RESULT
              <Info
                rows={[
                  [t("scan_info_shipment"), result?.data?.shipmentId],
                  [t("scan_info_tracking_id"), result?.data?.trackingId],
                  [t("scan_info_product"), result?.data?.productName || result?.data?.product],
                  [t("scan_info_status"), result?.data?.status],
                  [t("scan_info_origin"), result?.data?.source || result?.data?.origin || "—"],
                  [t("scan_info_destination"), result?.data?.destination || "—"],
                  [t("scan_info_integrity"), result?.data?.integrity?.status || "—"],
                ]}
              />
            )}
            ...
            <View style={styles.sheetBtns}>
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setScanned(false);
                  if (r.type === "device") {
                    navigation.navigate("DeviceDetails", { deviceId: r.data.deviceId });
                  } else {
                    navigation.navigate("ShipmentDetails", { shipmentId: r.data.shipmentId });
                  }
                }}
              >
                <Text style={styles.sheetGhostText}>
                  {result?.type === "device" ? t("scan_view_device") : t("scan_view_shipment")}
                </Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setScanned(false);
                  if (r.type === "device") {
                    navigation.navigate("AssignDevice", { deviceId: r.data.deviceId });
                  } else {
                    // Your backend doesn't expose a "confirm pickup" endpoint
                    // from the public trace API yet, so we just confirm verification here.
                    Alert.alert(t("scan_verified_title"), t("scan_verified_body"));
                  }
                }}
              >
                <Text style={styles.sheetPrimaryText}>
                  {result?.type === "device" ? t("scan_assign_to_shipment") : t("scan_verified")}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.sheet}>
            ...
            <Text style={styles.sheetTitle}>{t("scan_manual_title")}</Text>

            <TextInput
              value={manual}
              onChangeText={setManual}
              placeholder={mode === "device" ? "e.g. AGR-NODE-001" : "e.g. AGT-2026-00001"}
            ...
            <View style={styles.sheetBtns}>
              <TouchableOpacity
                style={styles.sheetGhost}
                onPress={() => { setManualOpen(false); setManual(""); }}
              >
                <Text style={styles.sheetGhostText}>{t("scan_cancel")}</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setManual("");
                  setScanned(false);
                  setTimeout(() => handleCode(code), 100);
                }}
              >
                <Text style={styles.sheetPrimaryText}>{t("scan_submit")}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
